...
# ── Configuration ────────────────────────────────────────────
RPC_URL="${RPC_URL:-https://testnet-rpc.chain.robinhood.com/}"
PRIVATE_KEY="${PRIVATE_KEY:-${AGENT_PRIVATE_KEY:-}}"
...
echo -e "  ${DIM}Deployer:   ${DEPLOYER}${RESET}"
...
cast send "$VAULT_ADDRESS" "setAgent(address)" "$DEPLOYER" \
  --rpc-url "$RPC_URL" \
  --private-key "$PRIVATE_KEY" \
  > /dev/null 2>&1

echo -e "  ${GREEN}✅ Agent registered: ${DEPLOYER}${RESET}"
...
# ─────────────────────────────────────────────────────────────
echo -e ""
echo -e "${CYAN}[Step 5/5]${RESET} Writing agent/.env..."
...
# ── Contract Addresses ───────────────────────────────────────
AEGIS_VAULT_ADDRESS=${VAULT_ADDRESS}
...
echo -e "${GREEN}${BOLD}║${RESET}  Agent:       ${DEPLOYER}  ${GREEN}${BOLD}║${RESET}"
